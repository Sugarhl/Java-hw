...
class NegativeHandlerTest {
    NegativeHandler checker = new NegativeHandler();
    ...
    void isNotCorrect() {
        ...
        assertFalse(checker.isNotCorrect(first, negative));
        assertTrue(checker.isNotCorrect(second, positive));
    ...
    void createMessage() {
        var first = getClass().getDeclaredFields()[1].getAnnotatedType().getAnnotation(InRange.class);
        assertEquals(checker.createMessage(first), "Value must be negative");
    }
}